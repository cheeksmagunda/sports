"""Thin Real Sports HTTP client for NHL (read-only).

Auth mirrors the NFL operator-seeded Playwright session pattern without
importing domain code from other sports packages. Uses the shared
REALSPORTS_STORAGE_STATE_B64GZ session on the NHL sport path only.
"""
...
DEFAULT_DEVICE_NAME = "nhl-oracle-dev-01"

log = logging.getLogger("nhl_oracle.ingest.realsports")
...
def _storage_state_candidates() -> list[Path]:
    ...
    override = (
        ...
        or os.environ.get("NHL_REALSPORTS_STORAGE_STATE", "").strip()
    )
    ...
    # under another sport app (path only; no cross-sport domain imports).
    paths.append(project_root().parent / "nfl-oracle" / "scraper" / "storage_state.json")
...
def token_cache_path() -> Path:
    override = (
        ...
        or os.environ.get("NHL_REALSPORTS_TOKEN_CACHE", "").strip()
    )
    if override:
...
def materialize_storage_state_from_env() -> Path | None:
    values = (
        ...
        ("NHL_REALSPORTS_STORAGE_STATE", os.environ.get("NHL_REALSPORTS_STORAGE_STATE", "")),
    )
    ...
    for key, value in configured:
        if _placeholder_secret(value):
            continue
        if key == "NHL_REALSPORTS_STORAGE_STATE" and not value.startswith("{"):
            continue
        try:
...
def load_cached_headers() -> RequestHeaders | None:
    ...
    if not path.exists():
        ...
        for sibling_name in ("nfl-oracle", "wnba-oracle"):
            sibling = project_root().parent / sibling_name / "scraper" / "request_token_cache.json"
            if sibling.exists():
                path = sibling
                break
        else:
...
def _device_uuid() -> str:
    return os.environ.get("NHL_DEVICE_UUID", "").strip() or "00000000-0000-4000-8000-000000000nhl"


def _device_name() -> str:
    return os.environ.get("NHL_DEVICE_NAME", "").strip() or DEFAULT_DEVICE_NAME
...
async def capture_live_headers(
    ...
    *,
    ...
    async with launch_chromium_session(
        ...
        if (not used_explicit) or os.environ.get("NHL_REALSPORTS_WRITE_STATE") == "1":
...
async def fetch_home_next(
    ...
    *,
    ...
    payload = response.json()
    if not isinstance(payload, dict):
        raise TypeError("home/next payload must be an object")
    return payload


async def fetch_home_day(
    client: httpx.AsyncClient,
    day: str,
    headers: RequestHeaders,
    *,
    ...
    response = await real_sports_get(
        client,
        f"{BASE}/home/{SPORT}/day/next",
        headers=wire,
        params={"day": day, "cohort": cohort},
        refresh_headers=refresh_headers,
    )
    payload = response.json()
    if not isinstance(payload, dict):
        raise TypeError("home/day/next payload must be an object")
    return payload
...
async def fetch_contest_meta(
    ...
    *,
    ...
    payload = response.json()
    if not isinstance(payload, dict):
        raise TypeError(f"contest meta for {contest_id} must be an object")
    return payload


async def fetch_contest_draftinfo(
    client: httpx.AsyncClient,
    contest_id: int,
    headers: RequestHeaders,
    *,
    ...
    payload = response.json()
    if not isinstance(payload, dict):
        raise TypeError(f"contest draftinfo for {contest_id} must be an object")
    return payload


async def try_fetch_contest_meta(
    client: httpx.AsyncClient,
    contest_id: int,
    headers: RequestHeaders,
    *,
    ...
    """Return contest meta or None when the contest id does not exist (404)."""
...
async def fetch_contest_stats(
    ...
    *,
    ...
    payload = response.json()
    if not isinstance(payload, dict):
        raise TypeError(f"contest stats for {contest_id} must be an object")
    return payload
