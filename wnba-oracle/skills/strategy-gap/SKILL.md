...
```sh
# From wnba-oracle/
uv run --frozen --package wnba-oracle python scripts/backtest_counterfactual.py --recent 20
```
...
```sh
uv run --frozen --package wnba-oracle python scripts/backtest_walkforward.py \
  --window 20 --step 5
```
...
```sh
uv run --frozen --package wnba-oracle python scripts/analyze_strategy_gap.py \
  --recent 20 --verbose
```
...
```sh
uv run --frozen --package wnba-oracle python scripts/loss_ledger.py --recent 20
```
...
uv run --frozen --package wnba-oracle python scripts/calibrate_starter_and_boost.py

# Calibrate contrarianstrength and optimizer settings
uv run --frozen --package wnba-oracle python scripts/calibrate_knobs.py
```

Record findings in the tracking issue and open a knob shadow via the knob-shadow
skill before changing any production default. Update `STATUS.md` only if the
active production configuration or risk state changes.
...
- **Boost calibration error > 20%**: the heuristic fallback
  ...
  players. Check the live model artifact identity and compare it with `STATUS.md`.
...
- Calibration scripts may write to `runs/` (gitignored); those outputs
  are not canonical. Record findings and decisions in the tracking issue or PR.
