...
def test_starter_unknown_fade_reduces_rank_for_unknowns() -> None:
    ...
    baseline = _apply_knob_overlay(p50=2.0, boost=1.0, features=features, overlay={})
    faded = _apply_knob_overlay(
        p50=2.0, boost=1.0, features=features, overlay={"starter_unknown_fade": 0.75}
    )
    assert baseline == 2.0
...
def test_starter_unknown_fade_does_not_touch_starters() -> None:
    ...
    baseline = _apply_knob_overlay(p50=2.0, boost=0.5, features=starter, overlay={})
    faded = _apply_knob_overlay(
        p50=2.0, boost=0.5, features=starter, overlay={"starter_unknown_fade": 0.5}
    )
...
def test_boost_tail_lift_applies_only_above_threshold() -> None:
    ...
    below = _apply_knob_overlay(p50=2.0, boost=1.5, features=features, overlay=overlay)
    at = _apply_knob_overlay(p50=2.0, boost=2.0, features=features, overlay=overlay)
    above = _apply_knob_overlay(p50=2.0, boost=2.5, features=features, overlay=overlay)
