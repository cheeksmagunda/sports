...
def test_offset_roundtrips_to_pred_mean() -> None:
    pred = 2.5
    for K in (2.0, 10.0):
        s = sample_joint_real_scores([_spec(pred, K)], 40000, CopulaConfig(seed=7, score_offset=K))
...
def test_smaller_offset_tightens_realized_spread() -> None:
    ...
    s10 = sample_joint_real_scores(
        [_spec(pred, 10.0)], 40000, CopulaConfig(seed=7, score_offset=10.0)
    )
