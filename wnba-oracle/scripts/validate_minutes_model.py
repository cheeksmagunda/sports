...
ALIAS = {"PHX": "PHO", "PHO": "PHO", "LV": "LVA", "LVA": "LVA", "POR": "PDX", "PDX": "PDX",
         "GSV": "GSV", "GS": "GSV"}
...
def main() -> None:
    ...
    print(f"corpus rows: {n_total}  |  matched to minutes: {len(matched)} "
          f"({len(matched)/n_total:.0%})")
    cov26 = j[j["slate_date"] >= "2026-01-01"]
    cov26m = cov26[cov26["min"].notna()]
    print(f"2026 rows: {len(cov26)}  matched: {len(cov26m)} ({len(cov26m)/max(len(cov26),1):.0%})")
    ...
    for pid, g in matched.groupby("player_id"):
        ...
        for i in range(len(g)):
            ...
            rows.append({
                "pid": pid, "date": dates[i], "n_prior": len(prior),
                "real": reals[i], "boost": boosts[i],
                "proj_min": pm, "proj_rate": pr, "actual_min": mins[i],
                "e_real_min": pm * pr, "e_real_boost": boost_prior(boosts[i]),
    ...
    print(f"  corr(real, minutes x rate)  = {corr('real','e_real_min'):+.3f}   MAE {mae('e_real_min'):.3f}")
    print(f"  corr(real, recent_real)     = {corr('real','recent_real'):+.3f}   MAE {mae('recent_real'):.3f}")
    print(f"  corr(real, boost_prior)     = {corr('real','e_real_boost'):+.3f}   MAE {mae('e_real_boost'):.3f}")
    print(f"  corr(real, proj_min alone)  = {corr('real','proj_min'):+.3f}")
    print(f"  corr(real, ACTUAL_min x rate)= {corr('real','e_real_perfectmin'):+.3f}   "
    ...
    for _d, g in P26.groupby("date"):
        if len(g) < 8:
            continue
        gg = g.merge(matched[["player_id", "slate_date", "card_boost"]],
                     left_on=["pid", "date"], right_on=["player_id", "slate_date"], how="left")
        b = gg["boost"].to_numpy()
    ...
    print(f"  minutes x rate : recovery {np.mean(rec_min):.2f}/5   Spearman {np.mean(rhos_min):+.3f}")
    print(f"  boost_prior    : recovery {np.mean(rec_boost):.2f}/5   Spearman {np.mean(rhos_boost):+.3f}")
