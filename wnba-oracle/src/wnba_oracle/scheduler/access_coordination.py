"""Durable coordination for one scarce external account's access windows.

Kept local to wnba-oracle rather than oracle-core (root AGENTS.md: prefer
keeping a capability local until production evidence and tests show a
stable provider-neutral interface). The mechanism here (a PostgreSQL
advisory-locked window table with account-wide spacing and a daily ceiling)
is provider-neutral in shape, but WNBA is its only consumer today and its
Postgres instance is WNBA-only; a second application with its own database
cannot share this table without its own migration and its own coordinator
call sites (see wnba-oracle/AGENTS.md's Jobs and production operations
section for the known nfl-oracle-worker gap this leaves).
"""
...
def try_acquire_access_window(
    ...
    *,
    ...
    """Atomically reserve a durable access window across processes."""
